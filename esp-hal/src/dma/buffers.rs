...
impl DmaTxStreamBuf {
    ...
    pub fn push(&mut self, data: &[u8]) -> usize {
        ...
        let available_buffer = &mut self.buffer[self.len_of_used_buffer..][self.buffer_write_offset..][..available_space];
    ...
    fn commit(&mut self, with_eof: bool) {
        ...
        let mut remaining_to_commit = &mut self.buffer[self.len_of_used_buffer..][..self.buffer_write_offset];
        ...
        while !remaining_to_commit.is_empty() {
            ...
            let desc = unwrap!(free_descriptors.next());
        ...
        let descriptors_to_link = self.descriptors.iter_mut()
            .rev()
...
impl DmaTxStreamBufView {
    ...
    pub fn push(&mut self, data: &[u8], set_eof: bool) -> usize {
        ...
        while !remaining_to_push.is_empty() {
            ...
            // info!("attempting reclaim? {} {} {} {} {} {} {} {}", self.free_descriptors, self.free_buffer_space, max_to_push, chunk_size, max_chunk_size, self.buffer.len(), self.buffer_idx, remaining_to_push.len());
            // panic!();
            ...
            self.descriptors[self.descriptor_idx.checked_sub(1).unwrap_or(self.descriptors.len() - 1)].next = descriptor;
    ...
    fn reclaim_from_dma(&mut self) {
        ...
        let mut buffer_checkpoint = unsafe {
            self.buffer.as_mut_ptr().add((self.buffer_idx + self.free_buffer_space) / self.buffer.len())
        };
        ...
        for descriptor in descriptors_to_reclaim {
            ...
            if descriptor.buffer >= buffer_checkpoint {
                ...
                self.free_buffer_space += unsafe { new_checkpoint.offset_from(buffer_checkpoint) } as usize;
                buffer_checkpoint = new_checkpoint;
            } else {
                self.free_buffer_space += unsafe { buffer_end.offset_from(buffer_checkpoint) } as usize;
                let new_checkpoint = unsafe { descriptor.buffer.add(descriptor.size()) };
                self.free_buffer_space += unsafe { new_checkpoint.offset_from(buffer_start) } as usize;
                buffer_checkpoint = new_checkpoint;
            }
            self.free_descriptors += 1;
        }
